...
async function scrapeHorrorMovies(year) {
  const url = "https://www.imdb.com/search/title/?genres=horror&sort=year,desc";
  if (year) {
...
builder.defineCatalogHandler(async ({ type, id, extra }) => {
  ...
  if (type === "movie" && id === "horror_movies") {
    let movies = await scrapeHorrorMovies(extra.year);

    // Sort by Year if Requested
    if (extra.year) {
      const filterYear = parseInt(extra.year);
      if (!isNaN(filterYear)) {
          movies = movies.filter((movie) => parseInt(movie.releaseInfo) === filterYear);
